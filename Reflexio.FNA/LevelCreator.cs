...
namespace Reflexio
{
    ...
    class LevelCreator
    {
        ...
        private static Dictionary<String, object> object_names = new Dictionary<String, object>();
        public static object CreateObjectFromName(string class_name, object[] args)
        {
            object obj;
            try
            {
                obj = Activator.CreateInstance(Type.GetType("Reflexio." + class_name), args);
            }
            catch
            {
                throw new System.Exception("The tag name " + class_name + " has no corresponding class. Please check level file.");
            }
            return obj;
        }
        ...
        public static void CallMethodFromName(object obj, string method_name, object[] arguments)
        {
            ...
            if (info == null)
                throw new System.Exception("The tag name " + method_name + " has no corresponding method. Please check level file.");
        ...
        public static object[] ParseArguments(string args)
        {
            ...
            for (int i = 0; i < string_args.Length; i++)
            {
                ...
                int inum;
                float fnum;
                if (object_names.ContainsKey(string_args[i]))
        ...
        public static object ParseNodes(XmlElement element)
        {
            object obj;
            object[] args = ParseArguments(element.GetAttribute("params"));
            if(element.HasAttribute("type"))
                obj = CreateObjectFromName(element.GetAttribute("type"), args);
            else
            ...
            foreach (XmlNode node in element.ChildNodes)
            {
                switch (node.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        {
                            object arg = ParseNodes((XmlElement)node);
        ...
        public static Level ParseLevelFromString(String str)
        {
            object_names.Clear();
            ...
            return (Level)ParseNodes(root);
        }

        public static Level ParseLevelFromFile(String xmlfilename)
        {
            object_names.Clear();
            XmlDocument xmldoc = new XmlDocument();
            XmlReader reader;
